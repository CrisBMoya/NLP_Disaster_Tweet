...
Keyword[which(is.na(DataDF$keyword))]
DataDF[which(is.na(DataDF$keyword)),]$keyword=Keyword[which(is.na(DataDF$keyword))]
...
Keyword[which(is.na(TestDF$keyword))]
TestDF[which(is.na(TestDF$keyword)),]$keyword=Keyword[which(is.na(TestDF$keyword))]
...
# Format Submission
Submission_v2=data.frame('id'=TestDF$id, 'target'=Test_Pred_Model)
write_delim(x=Submission_v2, path='NLP_v2.csv', delim=',')
...
if(RE){
  ...
  system('kaggle competitions submit -c nlp-getting-started -f NLP_v2.csv -m "Submission from API - NLP_v2"')
}
